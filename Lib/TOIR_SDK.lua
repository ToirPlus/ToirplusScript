...
function GetTrueAttackRange()
        return GetAttackRange(myHero.Addr) + GetOverrideCollisionRadius(myHero.Addr) + GetBoundingRadius(myHero.Addr)
end
...
local DamageLibTable = {
  ...
  ["Ashe"] = {
    ...
    {Slot = "R", Stage = 1, DamageType = 2, Damage = function(source, target, level) return ({200, 400, 600})[level] + source.MagicDmg end},
  ...
  ["Caitlyn"] = {
    ...
    {Slot = "R", Stage = 1, DamageType = 1, Damage = function(source, target, level) return ({250, 475, 700})[level] + source.TotalDmg end},
  },
  ...
  ["Draven"] = {
    ...
    {Slot = "R", Stage = 1, DamageType = 1, Damage = function(source, target, level) return ({175, 275, 375})[level] + 1.1 * source.BonusDmg end},
  },
  ...
  ["Ezreal"] = {
    ...
    {Slot = "E", Stage = 1, DamageType = 2, Damage = function(source, target, level) return ({80, 125, 175, 225, 275})[level] + 0.75 * source.MagicDmg + 0.5 * source.BonusDmg end},
    {Slot = "R", Stage = 1, DamageType = 2, Damage = function(source, target, level) return ({350, 500, 650})[level] + 0.9 * source.MagicDmg + source.BonusDmg end},
  },
  ...
  ["Jinx"] = {
    ...
    {Slot = "R", Stage = 1, DamageType = 1, Damage = function(source, target, level) return ({250, 350, 450})[level] + ({25, 30, 35})[level] / 100 * (target.MaxHP - target.HP) + 1.5 * source.BonusDmg end},
    --{Slot = "R", Stage = 2, DamageType = 1, Damage = function(source, target, level) return ({250, 350, 450})[level] + ({25, 30, 35})[level] / 100 * (target.MaxHP - target.HP) + 1.5 * source.BonusDmg end},
  },
  ...
  ["Lucian"] = {
    {Slot = "Q", Stage = 1, DamageType = 1, Damage = function(source, target, level) return ({85, 120, 155, 190, 225})[level] + source.BonusDmg + ({0.6, 0.7, 0.8, 0.9, 1})[level] * source.TotalDmg end},
  ...
  ["Tristana"] = {
    ...
    {Slot = "E", Stage = 1, DamageType = 1, Damage = function(source, target, level) return ({60, 70, 80, 90, 100})[level] + ({0.5, 0.65, 0.8, 0.95, 1.10})[level] * source.BonusDmg + 0.5 * source.MagicDmg end},
...
function minionManager:update()
	...
	_minionTable[MINION_ALLY] = {}
    _minionTable[MINION_ENEMY] = {}
    _minionTable[MINION_JUNGLE] = {}
    _minionTable[MINION_OTHER] = {}
...
function VPrediction:WayPointAnalysis(unit, delay, radius, range, speed, from, spelltype)
    ...
    if not unit.IsMove and GetDistance(myHero, unit) < range then
        HitChance = 3
    end
...
---------------------------------------------------------------------------------
function HPrediction:MenuValueDefault()
    self.menu = "<SDK> HPrediction"
    self.HPMenu_Buffer = self:MenuSliderFloat("Buffer distance", 10)
end

function HPrediction:OnDrawMenu()
    if Menu_Begin(self.menu) then
        self.HPMenu_Buffer = Menu_SliderFloat("Buffer distance", self.HPMenu_Buffer, 0, 20, self.menu)
        Menu_Text(string.format("Version: " .. tostring(self.version)))
        Menu_Text("Ported & Updated By: Dewblackio2")
        Menu_End()
    end
...
function HPrediction:MenuSliderFloat(stringKey, valueDefault)
  return ReadIniFloat(self.menu, stringKey, valueDefault)
end
...
function HPrediction:SpellCollision(HPskillshot, object, from, to)
  ...
  local objectAddRange = object.boundingRadius+self.HPMenu_Buffer
  local objectSpeed = object.MoveSpeed
  local to = Vector(to)
...
function HPrediction:OnUpdateBuff(source, unit, buff, stacks)
